<template>
	<view class="cmd-cell-item" :class="{'is-disabled': disabled,'no-border': !border }" @tap="$_click"
		...
		<view class="cmd-cell-item-body">
			...
			<view class="cmd-cell-item-right" v-if="!slotRight">
				...
				<view class="cmd-cell-icon-arrow-right">
					<switch v-if="showSwitch" :color="switchColor" :disabled='disabled' :checked="switchState"
						@change="$_switch" />
					<cmd-icon v-if="arrow" type="chevron-right" size="24" color="#C5CAD5"></cmd-icon>
				</view>
			</view>
		</view>
		<view v-if="addon2" v-text="addon2" class="cmd-cell-item-children" style="font-size: 24upx;color: #858b9c;">
		</view>
	</view>
</template>
